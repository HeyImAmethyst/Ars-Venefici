...
{
    public class WaveEffect : AbstractSpellEffect
    {
        ModEntry modEntry;
        ...
        public WaveEffect(ModEntry modEntry, ISpell spell, Vector2 pos, int piercing, float radius, int dur, int facingDirection) : base(modEntry, pos, dur)
        {
            ...
            if (Game1.activeClickableMenu == null && Game1.game1.IsActive)
            {
                ...
                if (facingDirection == 1 || facingDirection == 3)
                {
                    ...
                    for (int y = (int)(pos.Y - radius); y <= pos.Y + radius; ++y)
                    {
                        ...
                        Layer back_layer = GetGameLocation().Map.GetLayer("Back");
                        Layer buildings_layer = GetGameLocation().Map.GetLayer("Buildings");

                        Tile backLayerTile = back_layer.Tiles[newTilePos.GetTilePosX(), newTilePos.GetTilePosY()];
                        Tile buildingLayerTile = buildings_layer.Tiles[newTilePos.GetTilePosX(), newTilePos.GetTilePosY()];

                        Vector2 newAbsolutePos = Utils.TilePosToAbsolutePos(newTilePos.GetVector());

                        if (backLayerTile == null || (backLayerTile != null && !backLayerTile.Properties.ContainsKey("NPCBarrier")) || buildingLayerTile == null)
                        {
                            if (Game1.viewport.Contains(new xTile.Dimensions.Location((int)newAbsolutePos.X, (int)newAbsolutePos.Y)) || piercing > 0)
        ...
        public override void Draw(SpriteBatch spriteBatch)
        {
            ...
            //{
            //    Vector2 vec = new Vector2(r.X, r.Y + y);
            //    Vector2 absPos = Utils.TilePosToAbsolutePos(vec);
            ...
            if (facingDirection == 1 || facingDirection == 3)
            {
                for (int y = (int)(pos.Y - radius); y <= pos.Y + radius; ++y)
                {
                    ...
                    TemporaryAnimatedSprite sprite = new TemporaryAnimatedSprite("LooseSprites\\Cursors", new Rectangle(372, 1956, 10, 10), absPos, false, 1f / 500f, MagicHelper.Instance().GetColorForMagicType(spell)/*new Color(0, 48, 255, 127)*/)
                    {
                    ...
                    if (Game1.viewport.Contains(new xTile.Dimensions.Location((int)absPos.X, (int)absPos.Y)) || piercing > 0)
